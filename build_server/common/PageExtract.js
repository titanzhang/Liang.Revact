...
module.exports = {
  updateIndex: async function (product, existProduct) {
    try {
      ...
      productDO.setHash(product.urlHash).setTitle(product.title).setContent(product.description).setUrl(url).setPrice(product.price).setMsrp(product.msrp).setPriceChange(product.priceChange).setPriceChangePercent(product.priceChangePercent).setImage(product.image).setThumbnail(product.thumbnail);

      // Create solr history object
      const historyDO = new HistoryDO();
      historyDO.setHash(product.urlHash).setPrice(product.price);
