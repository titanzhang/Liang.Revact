...
class ContentSearch extends React.Component {
  ...
  fetchProductInfo(url) {
    ...
    fetch(`${this.props.productInfoApi}${url}?${new Date().getTime()}`).then(response => response.json()).then(json => {
  ...
  render() {
    return React.createElement(
      'div',
      null,
      React.createElement(NavHeader, { currentMenu: this.state.currentMenu }),
      React.createElement(
        'div',
        { className: 'main' },
        React.createElement(ProductGrid, { productList: this.state.productList, clickProduct: (url, product) => this.clickProduct(url, product) }),
        React.createElement(Pagination, this.state.pagination),
        React.createElement(ProductInfoDialog, { productInfo: this.state.productInfo, show: this.state.showProductDialog, hideModal: () => this.hideProductDialog() })
      ),
    ...
    return `
      ...
      <html>
        ...
        <body>
          ...
          <div id='content' class='container'>${ReactDOMServer.renderToString(React.createElement(ContentSearch, props))}</div>
