...
module.exports = (request, response) => {
  ...
  Promise.resolve(request).then(controller.run.bind(controller)).then(html => {
    response.send(html);
  }).catch(error => {
    // TODO: Error page
    response.send('error');
...
class SearchPageController {
  ...
  async search() {
    const products = await SolrProduct.DAO.getListByKeywordPriceP(this.keyword, this.pricePLow, this.pricePHigh, this.startIndex, this.numRows);
